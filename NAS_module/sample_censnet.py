...
import sys
sys.path.append("/workspace/") #Needed to be able to import from CensNet_module
sys.path.append("/workspace/CensNet_module/") #Needed to allow imported in code in CensNet_module
from CensNet_module.train import main as GNN_run
...
class GNN_Worker(Worker):
    ...
    def compute(self, config_id, config, budget, working_directory):
        ...
        tmp_cens_conf = {k:config[k] for k in config}
        tmp_cens_conf['epochs'] = budget
        tmp_cens_conf['out_dir'] = os.path.join(self.twig_config["out_dir"],
            self.run_id,
            "_".join(str(x) for x in config_id),
            str(random.random()))
        tmp_cens_conf['saved_name'] = "save.txt"

        # Run the model configuration and collect the results
        loss_score, acc = GNN_run(tmp_cens_conf)
